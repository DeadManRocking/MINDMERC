...
- **Rows by platform / bucket (verified live in Supabase):** **9 rows at session close (grows with each cron tick), platform=hackernews (9), bucket=W (9) → status `deferred`** (correct spec behavior — HN posts rarely state a budget, so the Stage-3 stated-budget gate classifies them W; only A rows get `queued`). post_ids all `hn_*`, dedupe working (no duplicates across the 3 harness passes).
